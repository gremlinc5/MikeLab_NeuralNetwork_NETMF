...
namespace AForge.Neuro
{
	...
	public class ActivationNeuron : Neuron
	{
        ...
        public JObject GetJson()
        {
            ...
            {
                arr.Add(this.weights[i]);
            }

            obj.Add("Weights", arr);

            obj.Add("Threshold", this.threshold);

            if (this.ActivationFunction.GetType() == typeof(BipolarSigmoidFunction))
            {
                obj.Add("Alpha", ((BipolarSigmoidFunction)this.ActivationFunction).Alpha);
            }
            else
            {
                throw new NotImplementedException("BipolarSigmoidFunction only");
            }
